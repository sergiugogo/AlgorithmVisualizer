...
import { dfs, Step } from '../../utils/graphAlgorithms';
import GraphBoard from './GraphBoard';

interface DFSAnimatorProps {
  graph: Graph;
  startNodeId: string;
}

export default function DFSAnimator({ graph, startNodeId }: DFSAnimatorProps) {
  const [animatedGraph, setAnimatedGraph] = useState<Graph>(() => resetGraphStatus(graph));
  const [activeEdge, setActiveEdge] = useState<string | null>(null);

  useEffect(() => {
    const steps = dfs(graph, startNodeId);
    let i = 0;
